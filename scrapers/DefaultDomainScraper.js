...
class DefaultDomainScraper extends BaseScraper {
  ...
  scrape($) {
    ...
    // check if it is a wprm recipe
      ...
      $(".wprm-recipe").length > 0 ||
      $("[class*='wprm-']").length > 0 ||
      $(".wprm-recipe-name").length > 0
    ) {
      ...
      $(".wprm-recipe-time-container").each((i, el) => {
        ...
        if (!label) {
          ...
          label = $(el).children(".wprm-recipe-time-header").text();
          console.log("HERE IS LABEL: ", label);
        }

        let recipeTime = $(el).children(".wprm-recipe-time").text();
    ...
    // check if it is mv
      ...
      $(".mv-create-ingredients")
        .find("li")
        .each((i, el) => {
          var text = $(el).text().replace(/\s\s+/g, " ").trim();
      ...
      let servings = $(".mv-create-nutrition-yield")
        .text()
        .trim()
        .toLowerCase();
    ...
    // check if it is ERS
      ...
      time.prep = $("*[itemprop = 'prepTime']").text()
        ? $("*[itemprop = 'prepTime']").text()
        : "";

      time.active = $("*[itemprop = 'cookTime']").text()
        ? $("*[itemprop = 'cookTime']").text()
        : "";

      time.total = $("*[itemprop = 'totalTime']").text()
    ...
    console.log(
      "Length of the Meta Title Property",
